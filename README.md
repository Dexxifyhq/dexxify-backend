...
In order, paste the contents of each file in `src/database/migrations/` into your PostgreSQL Editor and execute it:

1. `001_initial_schema.sql`
2. `002_provider_migration.sql`
...
```
src/
├── config/                  # Environment config (database, etc.)
├── database/
│   ├── entities/             # TypeORM entities
│   └── migrations/           # Numbered SQL migrations (applied manually via PostgreSQL Editor)
├── common/
...
Every business account has a **live** and **test** mode (`POST /api/v1/auth/mode` to switch), so integrations can be built and tested safely before going live
...
# Set the variables listed in .env.example (database connection, JWT secrets, etc.)
